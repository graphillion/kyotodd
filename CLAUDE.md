...
- Membership: `has_empty()` / `bddhasempty()` — check if ∅ ∈ F. `contains(s)` / `bddcontains()` — check if set s ∈ F. `is_subset_family(g)` / `bddissubset(f, g)` — check if F ⊆ G (early termination). `is_disjoint(g)` / `bddisdisjoint(f, g)` — check if F ∩ G = ∅ (early termination). `flatten()` / `bddflatten()` — union of all sets as a single-set ZDD.
