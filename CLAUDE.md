...
- Counting: `Card()` (uint64, saturating), `count()` (double), `exact_count()` (BigInt), `exact_count(ZddCountMemo&)`, `Lit()`, `Len()`, `min_size()` (minimum set size), `profile()` (set size distribution as `vector<BigInt>`), `profile_double()` (as `vector<double>`), `support_vars()` (variable numbers appearing in the ZDD), `element_frequency()` (per-variable frequency as `vector<BigInt>`, `result[v]` = number of sets containing variable v). Free function: `bddelmfreq()`.
