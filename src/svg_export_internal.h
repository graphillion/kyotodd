#ifndef KYOTODD_SRC_SVG_EXPORT_INTERNAL_H
#define KYOTODD_SRC_SVG_EXPORT_INTERNAL_H

// Internal header shared by the svg_export*.cpp implementation files.
// Declares the SVG layout engine (lanes, positions, writer) and per-DD-type
// helpers. Every symbol is defined inline so each translation unit gets its
// own copy — this file is intentionally NOT installed as a public header.
...
inline void svg_collect_expanded(
...
inline std::string make_terminal_svg(bddp f, const SvgParams& params) {
...
inline std::string node_label(bddvar var, const SvgParams& params) {
